...
from .models import Address, AddressField, Country, Locality, State
...
class AddressTestCase(TestCase):
    ...
    def test_unicode(self):
        self.assertEqual(
            str(self.ad1),
            '1 Some Street, Melbourne, Victoria 3000, Australia')
        self.assertEqual(
            str(self.ad_empty),
...
class AddressFieldTestCase(TestCase):
    ...
    def test_save(self):
        ...
        self.assertEqual(
            test.address.locality.postal_code,
            self.ad1_dict['postal_code'])
        self.assertEqual(
            test.address.locality.state.name,
            self.ad1_dict['state'])
        self.assertEqual(
            test.address.locality.state.code,
            self.ad1_dict['state_code'])
        self.assertEqual(
...
class FormAddressFieldTestCase(TestCase):
    ...
    def test_to_python_invalid_lat_lng(self):
        self.assertRaises(
            ValidationError, self.field.to_python, {
                'latitude': 'x'})
        self.assertRaises(
            ValidationError, self.field.to_python, {
                'longitude': 'x'})
